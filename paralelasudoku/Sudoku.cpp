...
//procedimiento que guardara posibles opciones para FuerzaBruta
/*void opcionesFuerza(Celda Matriz[][9],int valores[9]){
	for(int a=0;a<9;a++){
		valores[a]=0;
	}
	for(int i=0;i<9;i++){
...
/*funcion que calculara cuantas combinaciones posibles existen para aplicar FuerzaBruta
int combinaciones(Celda Matriz[][9]){
	...
	return (pow(contador1,contador2));
}*/
...
//funcion para ver que numero es posible insertar en alguna celda por FuerzaBruta
/*int cualFalta(Celda Matriz[][9],int i,int j){
	for(int i=0;i<10;i++){
		for(int j=0;j<10;j++){
			if(Matriz[i][j].Valor==0){
				for(int k=0;k<10;k++){
...
void clonar(Celda Matriz[][9], Celda BMatriz[][9]){
	for(int i=0;i<10;i++){
		for(int j=0;j<10;j++){
			...
			for(int k=0;k<10;k++){
				BMatriz[i][j].BloqueDisp[k]=Matriz[i][j].BloqueDisp[k];
...
void FuerzaBruta(Celda Matriz[][9],int e, int f){
	int sub, NumeroBuscado, n=0;
	bool puede1=false,puede2=false,puede3=false;
	for(int a=0;a<9;a++){
		//if(valores[a]!=0){
			NumeroBuscado=a+1;
			//cout << "numero buscado"<< NumeroBuscado <<  " lo busco " <<endl;
			while(e<9){
				while(f<9){
					if(Matriz[e][f].Valor==0 && Matriz[e][f].Disp[a]==1){
						puede1=RevisarFila(e,NumeroBuscado,Matriz);
						puede2=RevisarColumna(f,NumeroBuscado,Matriz);
						//sub=encontrarSubMatriz(e,f);
						//puede3=RevisarSubmatriz(sub,NumeroBuscado,Matriz);
						if(puede1==true && puede2==true){
							Matriz[e][f].Valor=NumeroBuscado;
							Matriz[e][f].Disp[a] = 0;
							cout<< "cambio un numero"<<endl;
			...
			/*for(int b=0;b<9;b++){
				...
				for(int c=0;c<9;c++){
					cout << "for c "<< endl;
					puede1=RevisarFila(i,NumeroBuscado,Matriz);
					puede2=RevisarColumna(j,NumeroBuscado,Matriz);
					sub=encontrarSubMatriz(i,j);
					puede3=RevisarSubmatriz(sub,NumeroBuscado,Matriz);
					if(puede1==true && puede2==true && puede3==true){*/
					/*if(Matriz[b][c].Valor==0 && Matriz[b][c].Disp[a]==1){
						Matriz[b][c].Valor=NumeroBuscado;
						Matriz[b][c].Disp[a] = 0;
						MostrarMatriz(Matriz);
					}
					while(n<1000){
	...
	//cout<<"FuerzaBruta aplicada" << endl;
}
...
int main(int argc, char* argv[]){
	...
	if(!finalizado(Matriz)){
		//opcionesFuerza(Matriz,valores);
		n=0;
		clonar(Matriz, BMatriz);

		for(int e=0;e<9;e++){
			for(int f=0;f<9;f++){
				FuerzaBruta(BMatriz,e,f);
				//cout << "hizo fuerza bruta con el valor inicial "<<e << " jop" <<endl;
				//cout << "hizo fuerza bruta con el valor inicial "<<f << " jop" <<endl;
